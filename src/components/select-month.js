...
function SelectMonth() {
  ...
  const prevDisable = maximumDate && utils.checkYearDisabled(year, true);
  const nextDisable = minimumDate && utils.checkYearDisabled(year, false);

  useEffect(() => {
    if (mainState.monthOpen) setShow(true);
    Animated.timing(openAnimation, {
      ...
      if (!mainState.monthOpen) setShow(false);
    });
  }, [mainState.monthOpen, openAnimation]);

  useEffect(() => {
    if (show) setYear(utils.getMonthYearText(mainState.activeDate).split(' ')[1]);
  }, [mainState.activeDate, utils, show]);

  const onSelectMonth = (month) => {
    if (show) {
      const date = new Date(utils.validYear(mainState.activeDate, year));
      const activeDate = month !== null ? new Date(date.setMonth(month)) : date;

      setMainState({
        type: 'set',
        monthOpen: (
          new Date(mainState.activeDate).getMonth() === activeDate.getMonth()
          && new Date(mainState.activeDate).getFullYear() !== activeDate.getFullYear()
        ),
        activeDate: utils.formatDate(activeDate),
      });
    }
  };
